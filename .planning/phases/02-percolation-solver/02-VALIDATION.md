...
**Nyquist rationale:** Plan 02-01 Task 1 creates `test_model_updates.py` (verifying model fields + output_helpers behaviors 1-6) and `test_percolation_solver.py` stubs (RED gate for Task 2). Plan 02-01 serves as the Wave 0 gate: test stubs for the percolation solver are written in Task 1 and must FAIL, then Task 2 implements production code to make them pass (GREEN). For Plan 02-02, channeling and fast mode are independent new modules whose tests are co-located with implementation -- the RED gate is provided by the 02-01 interface contracts (solve_accurate must exist before solve_fast can calibrate against it).
...
| 2-01-01b | 01 | 0 | model fields | unit | `cd brewos-engine && python -m pytest tests/test_model_updates.py -x -q` | W0 | pending |
...
- [ ] `tests/test_model_updates.py` — unit tests for model field additions (pressure_bar, channeling_risk) and output_helpers functions
